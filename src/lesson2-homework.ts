...
function excercise6(): void {
  ...
  console.log(`Exercise 6. Incremented age: ${JSON.stringify(incrementAge(userJohn))}`)
}
excercise6();
...
function excercise9() {
  ...
  class Book {
    ...
    constructor(title: string, publicationYear: number) {
      if (publicationYear > this.currentYear){
        throw new Error(`Publication year could not be in the future - ${publicationYear}`)
      }
      this.title = title;
    ...
    get age(): number{
      return this.currentYear - this.publicationYear;
    }
    ...
    revise(year: number): void {
      switch (true){
        case year < this.publicationYear:
          throw new Error(`Revised year - ${year},  cannot be less then current - ${this.publicationYear}.`)
        case year > this.currentYear:
          throw new Error(`Revised year - ${year} cannot be in the future.`)
        default: this.publicationYear = year
      }
    }
  }
  ...
  const hpInfo = {title: "Harry Potter and the sorcerers stone", publicationYear: 1998};
  const hpBook  = new Book(hpInfo.title, hpInfo.publicationYear);
  console.log(`Exersice 9. Book info: ${hpBook.getInfo()}`)
  ...
  hpBook.getInfo();
  console.log(`Exersice 9. Book age: ${hpBook.age}`)
  ...
  console.log(`Exersice 9. Book age after revise: ${hpBook.age}`)
  console.log(`Exersice 9. Book info after revise: ${hpBook.getInfo()}`)
  //hpBook.revise(2024) throws Error: Revised year - 2024 cannot be in the future.
  //hpBook.revise(1998) throws Error: Revised year - 1998,  cannot be less then current - 2001.
  ...
  class Magazine extends Book {
     ...
     * @param year - the year when book publicated. Could not be in the future
     * @param title - name of the book
     */
    constructor(day: number, month: number, year: number, title: string) {
      super(title, year);
      if (month < 1 || month > 12){
        throw new RangeError(`Month value is out of range: ${month}. Should be in the range [1-12] `)
      }
      if (day < 1 || day > 31){
        throw new RangeError(`Month value is out of range: ${day}. Should be in the range [1-31]`)
      }
      this.month = month;
      this.day = day
    }
    ...
    getInfo(): string {
      return super.getInfo().concat(`/${this.day.toString()}/${this.month.toString()}`);
    }
  }
